        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if serializer.validated_data.get('type') == 1:
                ...
                balance.filter(type=2).update(value=F('value') - serializer.validated_data.get('value'),
                                              date=serializer.validated_data.get('date'))
                return Response({'message': 'Текущий баланс пополнен, накопления уменьшены.'}, status=status.HTTP_201_CREATED)
            ...
            if serializer.validated_data.get('type') == 2:
                ...
                if len(balance.filter(type=2)) == 0:
                    ...
                    balance.update(value=F('value') + serializer.validated_data.get('value'),
                                   ...
                                   date=serializer.validated_data.get('date'),
                                   user=self.request.user)
                return Response({'message': 'Накопление добавлено.'}, status=status.HTTP_201_CREATED)
